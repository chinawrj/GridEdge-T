...
Android 同花顺模拟连接与 macOS 连接共享同一 durable outbox 和 `SimulationUiDriver` 执行语义，
但必须使用独立 adapter。每次读取或动作前必须显式绑定唯一 ADB emulator serial、实际 AVD 名称中
受审的 `THS` 标记、同花顺 package/version、当前 resumed activity package、页面标题“模拟炒股”、
唯一掩码账户和买入/卖出/撤单/持仓/查询控件集合；不能依赖 ADB 默认设备、模拟器窗口顺序、坐标常量
或仅凭顶部“模拟”标签判断账户。若当前仍是券商 A 股页、出现券商身份，或模拟身份/布局任一漂移，
必须在填写表单和资金动作前 fail closed。Android 页面未公开券商合同号时，只允许由日期、委托时间、
代码、方向、价格和数量的完整规范字段生成稳定 synthetic contract ID；相同规范字段出现多行时必须视为
歧义，绝不能按“最新一笔”或行号猜测。只读订单对账使用委托/查询证据，资金动作的撤单候选必须来自
顶部“撤单”tab 的可撤区域，不能从查询结果或不可撤的“其他”区域直接操作。撤单前必须重新打开目标行
的确认页并逐字段复核；已含撤单终态
的委托不得再次点击。成交明细只能归属到唯一兼容的已探测委托，缺少候选或多个候选都必须保持
AMBIGUOUS。Android adapter 的系统级资金动作必须在独立自动化夹具和受审真机布局通过后才能启用；
只读 identity/orders/fills probe 可以先行，但不得用未受审确认框推测提交成功。提交必须依次复核表单、
第一层委托确认、完整模拟账号哈希和最终成功合同号；任一层缺失或出现多个候选都不能重击。无日期订单页中，
只允许逐行详情完全一致、身份全局唯一且已经“全部撤单/全部成交/废单”的未来时刻行作为上一交易日残留显式
quarantine；该规则在盘前和盘中一致，任何非终态未来行仍然阻断启动。无人值守启动还必须固定唯一 AVD、
足够内存、完成开机和关闭系统动画，并在交易 worker 前通过
只读 identity/orders/fills/cancellable 健康门禁；模拟器或 APP 不可用时只允许有界恢复，不得重复资金动作。
