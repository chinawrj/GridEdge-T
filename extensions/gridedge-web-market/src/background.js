...
const REVIEWED_BUILD_ID = "collector-0.6.53-bounded-ack-source-presence-v1";
...
const sourcePresenceWatchdog = GridEdgeSourcePresence.createSourcePresenceWatchdog({
  readSettings: settings,
  readRemembered: async () => (await chrome.storage.local.get("reviewed_source_urls_v1")).reviewed_source_urls_v1,
  queryTabs: async () => await chrome.tabs.query({url: "https://quote.eastmoney.com/f1.html*"}),
  createTab: async (options) => await chrome.tabs.create(options),
  readCooldown: async (url) => (await chrome.storage.local.get(`source_presence_v1:${url}`))[`source_presence_v1:${url}`] ?? null,
  writeCooldown: async (now, url) => await chrome.storage.local.set({[`source_presence_v1:${url}`]: now}),
});

async function dispatchSourceHeartbeat() {
  void sourcePresenceWatchdog().catch(() => {});
...
async function deliverCapture(
  ...
  // Presence metadata must never delay durable evidence or DB-ACK delivery.
  void (async () => {
    const remembered = (await chrome.storage.local.get("reviewed_source_urls_v1")).reviewed_source_urls_v1 ?? [];
    if (!Array.isArray(remembered)) return;
    const nextRemembered = Array.from(new Set([...remembered, sender.url]));
    if (GridEdgeSourcePresence.validUrls(nextRemembered)) {
      await chrome.storage.local.set({reviewed_source_urls_v1: nextRemembered});
    }
  })().catch(() => {});
  const store = await GridEdgeDurable.status(database);
