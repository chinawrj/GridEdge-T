...
fn run() -> Result<()> {
    ...
    loop {
        ...
        let completion_gate = receipt
            .completion
            .map(|completion| {
                let (current, previous_current, observation_matches_today, released_bars_current) =
...
fn completion_gate_evidence(
    completion: gridedge_t::web_market::SourceCompletion,
    source_observation_gate: Option<(gridedge_t::web_market::SourceObservation, bool, bool)>,
    bars: &[MarketBar],
    now: NaiveDateTime,
    maximum_age_seconds: i64,
) -> Result<(bool, bool, bool, bool)> {
    let released_bars_current = bars
        .iter()
        .map(|bar| market_bar_is_current(bar.timestamp, now, maximum_age_seconds))
        .collect::<Result<Vec<_>>>()?
        .into_iter()
        .all(std::convert::identity);
    // Page liveness cannot certify trade coverage. Validate the completion
    // independently even when it releases no bars and an observation exists.
    let completion_current = market_watermark_is_current(
        Some(completion.covered_through_us),
        now,
        maximum_age_seconds,
    )?;
    let completion_contiguous = market_watermarks_are_contiguous(
        completion.previous_covered_through_us,
        completion.covered_through_us,
        maximum_age_seconds,
    )?;
    let completion_today = completion.session_date == now.date();
    let (current, previous_current, observation_matches_today) = source_observation_gate
        .map(|(observation, current, previous_current)| {
            (
                completion_current && current,
                completion_contiguous && previous_current,
                completion_today && observation.session_date == now.date(),
            )
        })
        .unwrap_or((completion_current, completion_contiguous, completion_today));
    Ok((
        current,
        previous_current,
        observation_matches_today,
