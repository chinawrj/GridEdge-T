...
fn full_fill_evidence_resolves_an_ambiguous_cancel_without_another_money_action() -> Result<()> {
    ...
    let mut outbox = submitted_outbox_with_quantity(&path, "6211087926", 27_500)?;
    let staged = outbox.staged_intents()?[0].clone();
    outbox.begin_cancellation(&staged.intent_id, &staged.request_sha256, "6211087926")?;
    ...
    let fills = (0..11)
        .map(|index| {
            ...
            fill_row(&format!("fill-{index:02}"), "6211087926", 2_500, price)
        })
        .collect::<Vec<_>>();
