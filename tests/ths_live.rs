...
fn rolling_upgrade_uses_a_fresh_contiguous_completion_until_source_observations_arrive() {
    ...
    assert!(gate.contains("completion_gate_evidence("));
    assert!(gate.contains("source_observation_gate"));
    let evidence = source
        .split_once("fn completion_gate_evidence(")
        .expect("independent observation and completion evidence")
        .1
        .split_once("fn completion_requires_recovery(")
        .expect("completion evidence helper end")
        .0;
    assert!(evidence.contains("completion.covered_through_us"));
    assert!(evidence.contains("completion.previous_covered_through_us"));
    assert!(evidence.contains("market_watermark_is_current("));
    assert!(evidence.contains("market_watermarks_are_contiguous("));
    assert!(
        evidence.contains(".map(|(observation, current, previous_current)|"),
        "a present source observation must remain the preferred liveness evidence"
    );
    assert!(evidence.contains("completion_current && current"));
    assert!(evidence.contains("completion_contiguous && previous_current"));
}
