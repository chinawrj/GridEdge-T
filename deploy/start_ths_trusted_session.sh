...
# The supervisor's immutable launcher supplies its frozen manifest identity.
# Recheck it INSIDE the same coordination lock used by runtime publication.
# An old supervisor can never cross an upgrade by verifying outside the lock.
if [ -n "${GRIDEDGE_SUPERVISOR_MANIFEST:-}" ]; then
  /opt/homebrew/bin/python3 "$deployment_root/bin/session_supervisor.py" \
    --manifest "$GRIDEDGE_SUPERVISOR_MANIFEST" \
    --manifest-sha256 "${GRIDEDGE_SUPERVISOR_MANIFEST_SHA256:?supervisor manifest SHA is required}" \
